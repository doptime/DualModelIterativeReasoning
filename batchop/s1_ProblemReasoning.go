...
func ParallelProblemReformulation(node *query.TreeNode) (msg *query.TreeNode, err error) {
	...
	c1 := node.NewChild("ProblemExploration")
	c1.UserMsg = message.UserMsg(prompt.String() + `Your goal is to do the Holistic Problem Exploration:
	...
	c2 := node.NewChild("ProblemExploration")
	c2.UserMsg = message.UserMsg(prompt.String() + `Your goal is to do the Intent Discovery:
	...
	c3 := node.NewChild("ProblemExploration")
	c3.UserMsg = message.UserMsg(prompt.String() + `Your goal is to do the Key Causual/context/Constraints Factors Identification:
	- List critical Causual factors that may influence the problem's solution.`)

	c4 := node.NewChild("ProblemExploration")
	c4.UserMsg = message.UserMsg(prompt.String() + `Your goal is to do the Key Result Identification:
	- List critical factors Result may be introduced in the problem's solution.`)
	err = query.AskLLMParallelly(c1, c2, c3, c4)
	if err != nil {
		return nil, err
	}
	CopyToClipboard(c1, c2, c3, c4)
...
`)
	...
	CopyToClipboard(msg)
	msgBest := msg.Clone()
	items := strings.Split(msg.Solution.Content, "Final Reformulated Problem Statement")
	if len(items) > 0 {
		msgBest.Solution.Content = "Problem Reformulated: " + items[1]
	}
	return msgBest, err
}
